...
* [GLOMAP](https://github.com/colmap/glomap) : Ce moteur de mapping global se présente comme une alternative plus rapide à COLMAP, promettant une accélération significative du processus de reconstruction. Son évaluation est actuellement en suspens à cause d'un [bug connu](https://github.com/colmap/glomap/issues/221). De plus, il reste dépendant de COLMAP pour les étapes préalables d'extraction et de matching des points d'intérêt (*features*).

* [CF-3DGS (COLMAP-Free 3DGS)](https://github.com/NVlabs/CF-3DGS) : Une piste que je souhaite évaluer pour s'affranchir totalement de la triangulation initiale lourde en apprenant conjointement les poses et la géométrie. Cependant, il s'agit d'un code de recherche non maintenu et dépendant de CUDA.

* [On the fly NVS](https://github.com/graphdeco-inria/on-the-fly-nvs?tab=readme-ov-file) : Développée par l'INRIA, cette approche propose d'entraîner les gaussiennes en temps réel. C'est la piste la plus prometteuse pour le projet, car elle permettrait une reconstruction dynamique et fluide durant le vol. Comme CF-3DGS, le code dépend de CUDA et est donc plus compliqué à utiliser sur ma configuration.
